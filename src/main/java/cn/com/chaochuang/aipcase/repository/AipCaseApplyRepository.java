/*
 * FileName:    DataUpdateRepository.java
 * Description:
 * Company:     南宁超创信息工程有限公司
 * Copyright:   ChaoChuang (c) 2015
 * History:     2015年3月27日 (LLM) 1.0 Create
 */

package cn.com.chaochuang.aipcase.repository;

import cn.com.chaochuang.aipcase.domain.AipCaseApply;
import cn.com.chaochuang.common.data.repository.SimpleDomainRepository;

/**
 * @author LJX
 *
 */
public interface AipCaseApplyRepository extends SimpleDomainRepository<AipCaseApply, Long> {

    /**
     * 根据远程系统基本编号获取AipCaseApply
     * 
     * @param rmCaseApplyId
     * @return
     */
    AipCaseApply findByRmCaseApplyId(Long rmCaseApplyId);
}
